...
describe("deserializeDesign", () => {
  ...
  it("opens a design saved with a pedestal blower as a plain blower", () => {
    // The pedestal blower was junked by the client (ADR-0043), but a design
    // autosaved before that still holds one. It must open rather than be
    // refused: the unit comes back on the square it was placed on, with no mast
    // under it and no cells claimed beneath it.
    const payload = JSON.parse(
      JSON.stringify(
        serializeDesign(
          designFromScene({
            parts: [{ id: "b1", type: "blower", cell: [2, 3, 2], dir: [0, 1, 0] }],
            obstacles: []
          }),
          TEST_APP_VERSION
        )
    ...
    payload.parts[0].pedestalFeet = 3;

    const result = deserializeDesign(JSON.stringify(payload));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.design.parts[0]).toMatchObject({ id: "b1", type: "blower", cell: [2, 3, 2] });
    expect("pedestalFeet" in result.design.parts[0]).toBe(false);
    expect(result.design.grid.query([2, 3, 2])).toBe("b1");
    // Nothing is held below it any more, so the squares the mast used to claim
    // are free for a tube to route through.
    expect(result.design.grid.query([2, 0, 2])).toBeUndefined();
  });
