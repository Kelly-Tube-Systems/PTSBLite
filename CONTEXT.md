...
| Part numbers | `partNo` in `src/data/parts.json` | **Real.** KTS numbers throughout: the blower (plain and on a pedestal, one part), terminal, tube, split sleeve and bend. The client, not the STEP folder, is the source where they disagree: the tube's number and the bend's are their 2026-09-17 corrections to the ones the folder carried, and the bend's is unsourced — they said to ship it and ask later. The bend's 3 ft radius is unaffected, being a spec fact rather than a catalogue value. See [ADR-0030](docs/adr/0030-the-terminal-and-pedestal-blower-take-their-real-numbers.md), [ADR-0036](docs/adr/0036-the-client-corrects-the-tube-part-number.md) and [ADR-0037](docs/adr/0037-the-bend-takes-the-clients-number-over-the-folders.md). |
