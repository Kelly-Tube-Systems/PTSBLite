...
| Part numbers | `partNo` in `src/data/parts.json` | **Real, all of them.** KTS numbers for the blower (plain and on a pedestal, one part), terminal, tube, split sleeve and bend. The client's own list of 2026-09-17 is the source and outranks the parts drop where they disagree. See [ADR-0036](docs/adr/0036-the-clients-list-supersedes-the-drop-and-nothing-is-invented.md). |
