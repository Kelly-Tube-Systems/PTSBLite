...
| Part numbers | `partNo` in `src/data/parts.json` | **Real; the bend's is unconfirmed.** KTS numbers for the blower (plain and on a pedestal, one part), terminal, tube and split sleeve. The client, not the STEP folder, is the source: the tube's number is their 2026-09-17 correction of the one the folder carried. The bend prints `ALP08401` on the client's instruction — they could not say whether it is the 3 ft radius part the app draws or the folder's 4 ft `ALP08404`, and told us to ship it and change it later. The radius stays 3 ft. See [ADR-0030](docs/adr/0030-the-terminal-and-pedestal-blower-take-their-real-numbers.md), [ADR-0036](docs/adr/0036-the-client-corrects-the-tube-part-number.md) and [ADR-0037](docs/adr/0037-the-bend-takes-the-clients-number-unconfirmed.md). |
