 ...
 *
 * Pass `--svg <path>` to also write a single mark back out as SVG. That is how
 * the baked paths get checked against the artwork by eye, and it is also where
 * `src/assets/kelly-systems-wordmark.svg` — the welcome screen's logo — comes
 * from:
 *
 *   node tools/bake-kelly-wordmark.mjs --svg src/assets/kelly-systems-wordmark.svg
 *
 * The screen cannot use the tile itself (three marks over a brick course) and an
 * SVG loaded as an image cannot reference paths in another file, so it needs a
 * file of its own. Baking it here rather than lifting a copy by hand is what
 * keeps it in step with the other two.
 */
...
if (svgFlag !== -1) {
  ...
  writeFileSync(
    ...
    `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">\n` +
      `  <!--\n` +
      `    One Kelly Systems mark, generated by tools/bake-kelly-wordmark.mjs from\n` +
      `    src/assets/kelly-systems-watermark.svg. Do not edit by hand: re-run the\n` +
      `    bake instead.\n\n` +
      `    Drawn in black, to be used as a mask so the colour comes from the\n` +
      `    stylesheet.\n` +
      `  -->\n` +
      paths.map((d) => `  <path d="${d}" />`).join("\n") +
      `\n</svg>\n`
  );
  process.stdout.write(`${target}: one mark\n`);
}
