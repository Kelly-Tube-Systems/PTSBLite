...
so the app has not moved and `ALP08404` is not its bend. The number the bend now prints, `ALP08401`,
came from the client on 2026-09-17 and he could not say which radius it describes
([ADR-0037](adr/0037-the-bend-takes-the-clients-number-unconfirmed.md)); if it turns out to be the
4 ft part, this question reopens as a geometry question before a catalog one.
...
**No part number in `src/data/parts.json` is invented any more; one is unconfirmed.** KTS delivered
...
([ADR-0036](adr/0036-the-client-corrects-the-tube-part-number.md)). The bend is `ALP08401`, given by
the client the same day and printed on his instruction although he could not say whether it is the
3 ft radius part the app draws or the folder's 4 ft `ALP08404`
([ADR-0037](adr/0037-the-bend-takes-the-clients-number-unconfirmed.md)). See also
[ADR-0029](adr/0029-real-part-numbers-arrive-in-part.md) and
[ADR-0030](adr/0030-the-terminal-and-pedestal-blower-take-their-real-numbers.md).

**A number taken on trust reads exactly like a verified one.** `ALP08401` sits among numbers KTS
confirmed and looks like one of them, so which line is unverified lives in ADR-0037 rather than in a
reader's memory. It is **published**: PTSBLite prints it into a BOM PDF that any member of the
public can download and keep, and nothing in the document says it was taken on the client's word.
That is the hazard [ADR-0013](adr/0013-lite-publishes-placeholder-part-numbers.md) named for
invented numbers, surviving its own supersession in a new form — the risk is no longer a number that
identifies nothing, but one that may identify the wrong bend. Issue #94.
